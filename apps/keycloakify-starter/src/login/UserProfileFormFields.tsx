...
export default function UserProfileFormFields(props: UserProfileFormFieldsProps<KcContext, I18n>) {
    const { kcContext, i18n, kcClsx, onIsFormSubmittableValueChange, doMakeUserConfirmPassword, AfterField } = props;
    ...
    function renderFields() {
        const sortedFields = ["email", "firstName", "lastName", "password", "password-confirm"];
        const sortedFieldStates = sortedFields
            .map(fieldName => formFieldStates.find(state => state.attribute.name === fieldName))
            .filter(state => state !== undefined);

        const passwordField = formFieldStates.find(s => s.attribute.name === "password");
        const currentPassword = typeof passwordField?.valueOrValues === "string" ? passwordField.valueOrValues : "";

        return (
            <>
                {sortedFieldStates.map((fieldState, index, arr) => {
                    if (fieldState.attribute.name === "firstName" && arr[index + 1]?.attribute.name === "lastName") {
                        return (
                            <div className="flex gap-4 mb-3" key={fieldState.attribute.name}>
                                <div className="flex-1">{renderSingleField(fieldState)}</div>
                                <div className="flex-1">{renderSingleField(arr[index + 1])}</div>
                            </div>
                        );
                    }

                    if (fieldState.attribute.name === "lastName") {
                        return null;
                    }

                    if (fieldState.attribute.name === "password-confirm") {
                        return (
                            <Fragment key={fieldState.attribute.name}>
                                {renderSingleField(fieldState)}
                                <PasswordRequirements password={currentPassword} kcClsx={kcClsx} show={true} />
                            </Fragment>
                        );
                    }

                    if (fieldState.attribute.name === "password" && !sortedFieldStates.some(s => s.attribute.name === "password-confirm")) {
                        return (
                            <Fragment key={fieldState.attribute.name}>
                                {renderSingleField(fieldState)}
                                <PasswordRequirements password={currentPassword} kcClsx={kcClsx} show={true} />
                    ...
                    return <Fragment key={fieldState.attribute.name}>{renderSingleField(fieldState)}</Fragment>;
                })}
            </>
        );
    }
